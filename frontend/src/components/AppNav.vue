<template>
  ...
  >
    <v-app-bar-nav-icon
      ...
      @click="expandDrawer"
    ></v-app-bar-nav-icon>
    <v-btn class="d-none d-lg-flex" @click="expandDrawer" fab icon>
      <UserConnect :profil="userConnect" :onlyProfilePicture="true"
    /></v-btn>
    <v-spacer class="d-xs-none"></v-spacer>
    <v-btn class="d-none d-md-flex" text rounded to="/thread">
      <v-icon>mdi-earth</v-icon><span class="ml-2">Place public</span>
    </v-btn>
    <v-btn class="d-md-none d-sm-flex" icon to="/thread"
      ><v-icon>mdi-earth</v-icon>
    </v-btn>
    <v-spacer class="d-xs-none"></v-spacer>
    <v-btn class="d-none d-md-flex" text rounded to="/privatethread">
      <v-icon>mdi-account</v-icon><span class="ml-2">Place personnel</span>
    </v-btn>
    <v-btn class="d-md-none d-sm-flex" icon to="/privatethread"
...
export default {
  ...
  watch: {
    status(newVal) {
      if (newVal === "success") {
        this.handleCreated().then(() => (this.profil = this.userConnect));
      }
    },
  },
  computed: {
